...
"""
# 编码器备份 用的一个改动的融合，我现在不想改动交叉注意力模块了
import logging
...
class BLIP2Cir(Blip2Base):
    ...
    def __init__(
        ...
        # self.t2t_text_proj = nn.Linear(self.Qformer.config.hidden_size, embed_dim) #投影层
        #--------------------------------------------------------------------------
    ...
    def forward(self, batch, fabric):
        ...
        # mean over all query tokens 改动5暂时注释掉平均
        # query_t2t_feat = query_t2t_feat.mean(dim=1)
        #改动5  在forward中添加注意力加权模块，后面考虑仿残次
        #----------------------------------------------------------------------------------------
        # weights_tt = self.attention_pool_text(query_t2t_feat)  # [bs, num_query_tokens, 1]
        # query_t2t_feat = (weights_tt * query_t2t_feat).sum(dim=1)  # [bs, hidden_size] # 3
        ...
        query_si_feat = query_si_feat + self.final_fuser(query_si_feat, diff_feat) #用残差结构 1
        ...
        if self.si_ti_weight > 0:
            ...
            #改动
            #------------------------------------------------------
            # loss += si_ti_loss * (1 - self.diff_loss_weight)
            ...
            # diff_loss = self.loss(diff_feat , tar_txt_feat , self.temp)
            # loss += diff_loss * self.diff_loss_weight
            #----------------------------------

        use_txt_loss = False
        if use_txt_loss and self.si_tc_weight > 0:
