...
def max_sim_q2t(vl_feats, tar_feats, batch_size=32):
    """
    vl_feats: (N, 32, 256)
    tar_feats: (M, 32, 256)
    return: (N, M)  每对 (i,j) 的 max_{q,k} <vl_iq, tar_jk>
    """
    N, Q, D = vl_feats.shape
    M, K, _ = tar_feats.shape
    out = torch.empty(N, M, dtype=vl_feats.dtype, device=vl_feats.device)

    # 外层按 vl 分批
    for i in range(0, N, batch_size):
        print(f'当前 {i}/4000')
        vl_b = vl_feats[i:i+batch_size]          # (B, Q, D)
        # 内层按 tar 分批，进一步降低峰值
        tar_batch_size = 256  # 可调
        max_ij = torch.full((vl_b.size(0), M), -float('inf'),
                            device=vl_feats.device, dtype=vl_feats.dtype)
        for j in range(0, M, tar_batch_size):
            tar_b = tar_feats[j:j+tar_batch_size]  # (T, K, D)
            # (B, Q, D) @ (T, K, D) -> (B, Q, T, K)
            sim = torch.einsum("iqe,jke->ijqk", vl_b, tar_b)
            print(f'sim.shape: {sim.shape}')
            # 立即 reduce 掉最后两维
            sim = sim.max(dim=-1)[0]
            sim = sim.max(dim=-1)[0]
            print(f'sim取max后的.shape: {sim.shape}')
            max_ij[:, j:j+tar_batch_size] = sim
        out[i:i+batch_size] = max_ij
    return out
...
class TestCirr:
    ...
    def __call__(model, data_loader, fabric):
        ...
        if fabric.global_rank == 0:
            ...
            # print(f"tar_feats.shape: {tar_feats.shape}")
            # batch_size = 100
            # sims_q2t = []
            ...
            # sims_q2t = sims_q2t.max(dim=-1)[0]
            sims_q2t = max_sim_q2t(vl_feats, tar_feats, batch_size=32)
